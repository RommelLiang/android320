...
//		return m.matches();
	...
	public static String removeYear(String string) {
		Pattern compile = Pattern.compile("(20\\d{2}[年])");
		Matcher m = compile.matcher(string);
		String name = string;
		if (m.find()) {
			Log.e( "removeYear: ", m.group(0));
			return name.replace(m.group(0), "");
		}
		Log.e( "removeYear: ", name);
		return name;
	}
