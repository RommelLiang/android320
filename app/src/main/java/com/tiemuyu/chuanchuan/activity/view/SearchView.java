...
import com.tiemuyu.chuanchuan.activity.NewSearchActivity;
...
public class SearchView extends LinearLayout {
	...
	/*UI组件*/
	private TextView tv_clear, tv_search_btn;
...
//                            getCurrentFocus().getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);
					...
					intent.putExtra("key", et_search.getText() + "");
	...
	private void initView() {
		...
		et_search = (ClearEditText) findViewById(R.id.cet_search);
	...
	private void queryData(String tempName) {
		...
		//模糊搜索
		mCursor = helper.getReadableDatabase().rawQuery(
				"select id as _id,name from records where name like '%" + tempName + "%' order by id desc ", null);
		// 创建adapter适配器对象,装入模糊搜索的结果
		adapter = new SimpleCursorAdapter(context, android.R.layout.simple_list_item_1, mCursor, new String[] {"name"},
